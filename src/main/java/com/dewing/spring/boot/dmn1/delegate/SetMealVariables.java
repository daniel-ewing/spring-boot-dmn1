...
@Component
@Slf4j
public class SetMealVariables implements JavaDelegate {
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        ...
        variables = setVarRandomValue(variables,"ageOfGuest", (int)Math.floor((Math.random() * 100) + 1));
        variables = setVarRandomValue(variables,"timeOfDay", (int)Math.floor((Math.random() * 24)));
        variables = setVarRandomValue(variables,"hunger", (int)Math.floor((Math.random() * 10) + 1));
